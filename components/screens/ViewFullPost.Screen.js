...
export default class ViewFullPost extends React.Component {
  ...
  deletePost() {
    try{
    firebase.database().ref('Posts/' + this.props.route.params.post.id).remove();
    ...
    return (
      <ScrollView style={globalStyles.container}>
        ...
        { post.start == post.end?
          ...
          <Text style={globalStyles.text}>{formatDate(post.start) + ' - ' + formatDate(post.end)}</Text>
          </View>
        }
        ...
        <Text style={globalStyles.text}>{formatTime(post.start) + ' - ' + formatTime(post.end)}</Text>
        </View>
          ...
          <View style={{ flexDirection: 'row', flex: 2}}>
					...
					<View style={{flex: 1, margin: 5}}>
					<Button onPress={() => this.deletePostWarning()} color="error" title="Delete Post" />
					</View>
...

          {
            commentsReversed.map((l, i) => (
              <ListItem key={i} bottomDivider>
                ...
                <ListItem.Content>
                <ListItem.Subtitle>{l.name + " " + formatDate(l.date) + ' ' + formatTime(l.date)}</ListItem.Subtitle>
